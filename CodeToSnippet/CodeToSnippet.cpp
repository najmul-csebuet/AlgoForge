...
const string cppJson = "~/.config/Code/User/snippets/cpp.json";
const string snippetFileName = "Snippets.txt";

ifstream fin;
ofstream fout(snippetFileName);
...
void handleSingleLine(string prefixAndBody) {
  ...
  if (body.find(";") == string::npos) {
    ...
    // ; found. so we can write $0 in next line
    fout << "\t\t\t\""
         << "$0"
         << "\",\n";
  }
...
void generate(string inputFileName) {
  ...
  while (fin.good()) {
    ...
    if (firstLine.find("$") == string::npos) {
      // this is multi line snippet
      // as we do not found $ in line 1
